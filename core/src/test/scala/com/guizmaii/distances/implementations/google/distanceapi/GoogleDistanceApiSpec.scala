...
class GoogleDistanceApiSpec extends WordSpec with Matchers with ScalaFutures with BeforeAndAfterEach {
  ...
  lazy val distanceApi: DistanceApi[IO] = GoogleDistanceApi(geoContext)
  ...
  "GoogleDistanceApi.distance" should {
    "says that Paris 02 is nearest to Paris 01 than Paris 18" in {
      val paris01 = geocoder.geocodePostalCode(PostalCode("75001")).unsafeRunSync()
      val paris02 = geocoder.geocodePostalCode(PostalCode("75002")).unsafeRunSync()
      val paris18 = geocoder.geocodePostalCode(PostalCode("75018")).unsafeRunSync()
      ...
      val from01to02 = distanceApi.distance(paris01, paris02, Driving :: Nil).unsafeRunSync()
      val from01to18 = distanceApi.distance(paris01, paris18, Driving :: Nil).unsafeRunSync()
