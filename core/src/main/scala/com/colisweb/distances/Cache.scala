...
abstract class Cache[F[_]: Async](ttl: Option[Duration]) {
  ...
  def cachingF[V](f: F[V], decoder: Decoder[V], encoder: Encoder[V], keys: Seq[Any]): F[V] =
    innerCache
      .cachingF(keys: _*)(ttl)(f.map(encoder.apply))
      .flatMap(json => Async[F].fromEither(decoder.decodeJson(json)))

  def caching[V](v: V, decoder: Decoder[V], encoder: Encoder[V], keys: Seq[Any]): F[V] =
    innerCache
      .caching(keys: _*)(ttl)(encoder.apply(v))
      .flatMap(json => Async[F].fromEither(decoder.decodeJson(json)))

  def get[V](decoder: Decoder[V], keys: Seq[Any]): F[Option[V]] =
    innerCache
      .get(keys: _*)
      .flatMap(_.traverse(json => Async[F].fromEither(decoder.decodeJson(json))))

  def remove(keys: Seq[Any]): F[Unit] =
...
object Cache {
  ...
  type CachingF[F[_], V] = (F[V], Decoder[V], Encoder[V], Seq[Any]) => F[V]

  // This is a caching function for a value V.
  type Caching[F[_], V] = (V, Decoder[V], Encoder[V], Seq[Any]) => F[V]
  ...
  type GetCached[F[_], V] = (Decoder[V], Seq[Any]) => F[Option[V]]
